...
void mips_gen(quad **q, struct scope* s) {
	FILE* file = NULL;
...
void mips_vars(struct scope* s, FILE* f) {

	if(s == NULL) return;

	struct symbol *sym;

	for (sym = s->tds; sym != NULL; sym = sym->next) {
...
void mips_write(quad *t, FILE *file) {
	...
	switch(t->quad_type) {
		...
		case AFFEC_BINARY_PLUS:
			fprintf(file, "\tadd %s, %s, %s\n", t->res->id, t->operande1->id, t->operande2->id);
			break;
		...
		case AFFEC_BINARY_AND:
			fprintf(file, "\tand %s, %s, %s\n", t->operande1->id, t->operande2->id, t->res->id);
			break;
