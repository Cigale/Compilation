...
Commands : Const Input Output Global Local Blankline Code
		 ;

Const : TK_CONST '{' '$' Declarations '$' '}' {printf("Constant found\n");}
	  | {printf("No Constant found\n");}
	  ;

Input : TK_INPUT '{' '$' Declarations '$' '}' {printf("Input found\n");}
	  ;

Output : TK_OUTPUT '{' '$' Declarations '$' '}' {printf("Output found\n");}
	   ;

Global : TK_GLOBAL '{' '$' Declarations '$' '}' {printf("Global found\n");}
	   ;

Local : TK_LOCAL '{' '$' Declarations '$' '}' {printf("Local found\n");}
	  ;

Blankline : TK_BLK {printf("Blankline found\n");}
		  ;

Declarations : TK_EMPTY
			 | Declarations_List
			 ;

Declarations_List : Parameter ',' Declarations_List 
				  | Parameter
				  ;

Parameter : TK_IDENT TK_IN TK_TYPE Table
		  ;
...
Instruction : Affectation TK_ENDINST {printf("Affectation found\n");}
...
Affectation : '$' TK_IDENT TableValue TK_LEFT Expression '$'
		| '$' TK_IDENT TK_LEFT Expression '$'
		;

While : TK_WHILE '{' '$' Condition '$' '}' '{' Code '}'
	  ;

Repeat : TK_REPEAT '{' '$' Condition '$' '}' '{' Code '}'
	   ;

If : TK_IF '{' '$' Condition '$' '}' '{' Code '}'
   ;

Eif : TK_EIF '{' '$' Condition '$' '}' '{' Code '}' '{' Code '}'
	;

For : TK_FOR '{' Affectation TK_TO '$' Expression '$' '}' '{' Code '}'
	;

Mbox : TK_MBOX '{' TK_IDENT '(' '$' Expression '$' ')' '}'
	 | '$' TK_MBOX '{' TK_IDENT '(' '$' Expression '$' ')' '}' '$'
	 ;

Condition 	: Expression TK_OR Condition			{
														printf("Mem : %d\n", $3);
														printf("Mem.code : %d\n", $3.code);
														printf("Mem.addr : %d\n", $3.addr);
